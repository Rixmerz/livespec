"""v0.15: auto-derived per-RF test coverage from the call graph.

`audit_coverage` (and the shared `compute_coverage` helper) now derive
per-RF test coverage automatically: a test function whose forward call
graph reaches an RF's `implements` symbol within `_TEST_REACH_DEPTH` hops
credits that RF — no hand-linked `relation='tests'` row required. Explicit
...
@pytest.mark.asyncio
async def test_rf_coverage_derived_from_call_graph(workspace):
    """A test function that DIRECTLY calls an RF's implementing symbol
    ...
    async with Client(mcp) as c:
        ...
        await c.call_tool(
            "link_rf_symbol",
            {"rf_id": "RF-001", "symbol_qname": "pkg.feature.implementer"},
        )
        out = (await c.call_tool("audit_coverage", {})).data

    by_id = {r["rf_id"]: r for r in out["rf_coverage"]}
    assert "RF-001" in by_id, f"RF-001 missing from rf_coverage: {out['rf_coverage']}"
    entry = by_id["RF-001"]
    ...
    # Rollups present and reflect the credited RF.
    assert out["rfs_with_any_test_coverage"] >= 1
    assert out["avg_test_coverage"] > 0
    assert out["counts"]["rfs_with_any_test_coverage"] >= 1
    assert out["counts"]["avg_test_coverage"] > 0


@pytest.mark.asyncio
async def test_rf_coverage_explicit_link_without_call_edge(workspace):
    ...
    async with Client(mcp) as c:
        await c.call_tool("index_project", {})
        await c.call_tool(
            "create_requirement", {"rf_id": "RF-002", "title": "Harness-tested"}
        )
        await c.call_tool(
            "link_rf_symbol",
            {"rf_id": "RF-002", "symbol_qname": "pkg.feature.implementer"},
        )
    ...
    by_id = {r["rf_id"]: r for r in out["rf_coverage"]}
    assert "RF-002" in by_id, f"RF-002 missing: {out['rf_coverage']}"
    entry = by_id["RF-002"]
    assert entry["test_coverage_ratio"] > 0, (
...
@pytest.mark.asyncio
async def test_rf_coverage_zero_when_nothing_reaches_impl(workspace):
    """An RF with impl symbols that no test reaches and no explicit link →
    ...
    async with Client(mcp) as c:
        await c.call_tool("index_project", {})
        await c.call_tool(
            "create_requirement", {"rf_id": "RF-003", "title": "Untested"}
        )
        await c.call_tool(
            "link_rf_symbol",
            {"rf_id": "RF-003", "symbol_qname": "pkg.feature.untested_impl"},
        )
        out = (await c.call_tool("audit_coverage", {})).data

    by_id = {r["rf_id"]: r for r in out["rf_coverage"]}
    assert "RF-003" in by_id, f"RF-003 missing: {out['rf_coverage']}"
    entry = by_id["RF-003"]
...
@pytest.mark.asyncio
async def test_rf_coverage_backward_compat_explicit_fields_intact(workspace):
    """The v0.8 explicit-link fields (`rf_test_coverage` /
    `counts.rfs_with_test_coverage`) must remain exactly as before alongside
    the new auto-derived `rf_coverage` block."""
    pkg = workspace / "pkg"
    pkg.mkdir()
    (pkg / "__init__.py").write_text("")
    (pkg / "feature.py").write_text(
    ...
    assert out["counts"]["rfs_with_test_coverage"] == 1, (
        f"explicit rfs_with_test_coverage must be intact: {out['counts']}"
    )
    assert any(
        r["rf_id"] == "RF-200" and r["test_count"] == 1
        for r in out["rf_test_coverage"]
    ), f"RF-200 must still be in rf_test_coverage: {out['rf_test_coverage']}"
    # New auto-derived block coexists.
    by_id = {r["rf_id"]: r for r in out["rf_coverage"]}
    assert "RF-200" in by_id
