...
def render_markdown(
    *,
    ...
    lines.append(f"**{len(specs_touched)}** spec(s) touched:")
    lines.append("")
    ...
    lines.append("")
    under_tested = [r for r in specs_touched if float(r.get("test_coverage_ratio") or 0) < 0.5]
    if under_tested:
        ids = ", ".join(f"`{r['spec_id']}`" for r in under_tested[:12])
        lines.append(f"> **Heads-up:** low test coverage on {ids}.")
        lines.append("")
    lines.append(
        "_Generated by [livespec](https://github.com/Rixmerz/livespec) `scripts/pr_diff_impact.py`_"
    )
    lines.append("")
    return "\n".join(lines)
