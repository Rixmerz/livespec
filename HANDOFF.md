...
Tests **294 default + 3 embeddings = 297**. Schema **v8** (sin cambios).
...
| P2 | CLI headless: `livespec-mcp index <path> [--force\|--embed]` / `status <path>` → JSON stdout, errors stderr rc=1. Mismo pipeline que el tool (`run_index_pipeline` compartido en tools/indexing.py). Sin args / `serve` = server stdio intacto. 4 tests |
