...
explorer ganan cobertura derivada). Ejecutado vía super-workflow jig
...
jig falsea fuera del venv `uv` (corrido manual: 316 passed).
...
`export_explorer`. Ejecutado vía workflow jig `rf-explorer-v015-graph`
...
configurar el validator jig para `uv run pytest` (fuera de este repo).
